...
9. **Mutation keys are role-scoped.** Every mutation endpoint on the live process gates on `ALPHA_LIVE_KEY`; every mutation on replay gates on `ALPHA_REPLAY_KEY`. There is no shared admin key. Reads optionally gate on `ALPHA_API_KEY`. Schwab live trading keeps a separate `ALPHA_OMS_KEY` (real-money gate, kept distinct from the mutation key). See `operations.md` §"Authentication".
...
11. **Two-process role boundary.** AlphaDB runs as `alphaserver-live` (`:8080`) + `alphaserver-replay` (`:8081`) sharing one OMS DB. Trading writes (`place_order`, `place_spread`, `close_spread`, …) are gated by `account_key` via `RoleGuard` — **every write endpoint exists on both ports**; route by account, not by endpoint. Accounts in `ALPHADB_LIVE_ACCOUNT_KEYS` (`paper`, `schwab-sim`) → `:8080`. All other accounts → `:8081`. Wrong account/port returns `400 WRONG_ROLE`. The `/v1/admin/as-of-date` routes (GET/POST/DELETE) are **physically absent** on the live process — calls to `:8080` return `404`, not `400`. `replay-day` and `simulate-outcome` exist on both ports but only succeed for replay-domain accounts. Background loops only attach in the live process. Only the live process migrates the OMS schema; replay verifies `schema_version` matches `oms.CurrentSchemaVersion` at startup and via `/v1/trading/health` (503 on skew). See `oms_replay.md` §"Two-process role boundary".
