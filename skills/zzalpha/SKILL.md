...
11. **Two-process role boundary.** AlphaDB runs as `alphaserver-live` (`:8080`) + `alphaserver-replay` (`:8081`) sharing one OMS DB. Writes are gated by `account_key` via `RoleGuard` — **every write endpoint exists on both ports**; route by account, not by endpoint. Accounts in `ALPHADB_LIVE_ACCOUNT_KEYS` (`paper`, `schwab-sim`) → `:8080`. All other accounts → `:8081`. Wrong port returns `400 WRONG_ROLE`. The time-machine endpoints (`replay-day`, `simulate-outcome`, `as-of-date` set/clear) are role-pinned to `replay` regardless of account and must use `:8081`. Background loops only attach in the live process. See `oms_replay.md` §"Two-process role boundary".
